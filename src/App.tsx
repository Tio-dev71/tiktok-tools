...
export default function App() {
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      // Step 2.5: Generate images for all sentences
      if (workingMediaItems.length === 1) {
        setStatus('generating_images');
        
        // Add a small initial delay to avoid hitting rate limits after audio generation
        await new Promise(resolve => setTimeout(resolve, 5000));
        
        const badgeForImage = finalHook;

        // Split script into sentences, filter out empty ones
        const sentences = generatedScript.split(/[.!?]\s+/).filter(s => s.trim().length > 5);
        
        // Generate images for EVERY sentence SEQUENTIALLY to avoid rate limits
        const generatedImages = [];
        let lastSuccessfulImage = workingMediaItems[0];
        ...
        for (let i = 0; i < sentences.length; i++) {
          ...
          try {
            let base64;
            if (i === 0) {
              // First sentence uses the source image if available
              base64 = await gemini.generateImage(sentence, workingMediaItems[0].base64, badgeForImage);
            } else {
              // Subsequent sentences generate new images
            ...
            generatedImages.push(newItem);
            lastSuccessfulImage = newItem;
          } catch (e) {
            console.error(`Failed to generate image for sentence ${i}:`, sentence, e);
            // Fallback to last successful image to keep the video consistent
            generatedImages.push(lastSuccessfulImage);
          }
          // Add a larger delay between requests to respect rate limits (3 seconds)
          if (i < sentences.length - 1) {
            await new Promise(resolve => setTimeout(resolve, 3000));
          }
        }
        finalMediaItems = generatedImages;
      }

      // Step 3: Generate Video Locally (No Veo needed)
      setStatus('video');
      const generatedVideoUrl = await generateVideoLocally(finalMediaItems, audioBase64, finalHook, maskUrl, generatedScript);
  ...
  const generateVideoLocally = async (mediaSources: { 
    ...
    endTime?: number;
    duration?: number;
  }[], audioBase64: string, title: string, currentMaskUrl: string, fullScript: string): Promise<string> => {
      ...
      try {
        ...
        const renderSingleMedia = (index: number, progress: number, mediaElapsed: number) => {
          ...
          if (index === 0) {
            ...
            const baseScale = Math.min(maxDrawW / sourceW, maxDrawH / sourceH);
            ...
            // Full bleed logic for subsequent items
        ...
        const sentences = fullScript.split(/[.!?]\s+/).filter(s => s.trim().length > 0);
        ...
        if (sentences.length > 1) {
          ...
          const remainingChars = remainingSentences.reduce((acc, s) => acc + s.length, 0);
          
          itemDurations = [firstItemDuration];
          remainingSentences.forEach(s => {
            itemDurations.push((s.length / Math.max(1, remainingChars)) * remainingDuration);
          });
        } else {
        ...
        const getMediaInfo = (elapsed: number) => {
          if (loadedMedia.length === 1) {
            return { index: 0, progress: elapsed / duration, mediaElapsed: elapsed, isTransitioning: false, nextIndex: -1, transitionAlpha: 0 };
          }
          ...
          // Transition logic
          const isTransitioning = index < loadedMedia.length - 1 && mediaElapsed > (itemDuration - transitionDuration);
          const nextIndex = isTransitioning ? index + 1 : -1;
          const transitionAlpha = isTransitioning ? (mediaElapsed - (itemDuration - transitionDuration)) / transitionDuration : 0;

          return { index, progress, mediaElapsed, isTransitioning, nextIndex, transitionAlpha };
        };
        ...
        const animate = (now: number) => {
          ...
          try {
            ...
            loadedMedia.forEach((m, i) => {
              if (m.type === 'video' && i !== info.index && i !== info.nextIndex) {
        ...
        ctx.fillStyle = themeColor;
        ctx.fillRect(0, 0, canvas.width, canvas.height);
        renderSingleMedia(firstInfo.index, firstInfo.progress, firstInfo.mediaElapsed);
            ...
            lines.forEach((l, lineIndex) => {
              ...
              lineWords.forEach((word, wordIndex) => {
                const globalWordIndex =
                  lines.slice(0, lineIndex).reduce((acc, curr) => acc + curr.trim().split(' ').length, 0) + wordIndex;
